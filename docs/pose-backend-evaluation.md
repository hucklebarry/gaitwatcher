...
The diagnostic-only `safetyPrimitive` does not alter `bodyState`, write records, send notifications, or infer falls. It uses confident shoulders and hips only, where available. Real YOLOv8n person boxes and separate box diagnostics were subsequently added; see [person-box temporal diagnostics](person-box-temporal.md). Those diagnostics remain camera-relative debug evidence, not fall inference.
